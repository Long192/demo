...
public interface FriendRepository extends JpaRepository<Friend, Long> {
    @Query("SELECT f FROM Friend f WHERE (f.friendReceiver.id = :user OR f.friendRequester.id = :user) AND f.status = 'accepted' AND ((LOWER(f.friendReceiver.email) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(f.friendRequester.email) LIKE LOWER(CONCAT('%', :search, '%'))) OR (LOWER(f.friendReceiver.email) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(f.friendRequester.email) LIKE LOWER(CONCAT('%', :search, '%'))))")
